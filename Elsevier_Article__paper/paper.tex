...
\label{sec:related}
Oversampling for imbalanced data was popularized by SMOTE \citep{chawla2002}, which interpolates between a sample and its $k$-nearest neighbors. Because raw pixel interpolation yields blurry, ghosting artifacts, subsequent works shifted to deep generative models. Generative Adversarial Networks (GANs) have been adapted for this purpose (e.g., BAGAN, GAMO), but they are notoriously difficult to train on highly imbalanced datasets due to mode collapse.
...
DeepSMOTE \citep{dablain2021} consists of a three-phase pipeline. First, an encoder-decoder architecture is trained on the imbalanced dataset. The network uses a reconstruction loss alongside a cyclic permutation penalty term that forces the embeddings of samples from the same class closer together. Second, the trained encoder projects the minority class images into a high-dimensional continuous latent space (e.g., $d=600$). For each minority latent vector, standard SMOTE selects a neighbor from its $k=5$ nearest neighbors and generates a synthetic vector via linear interpolation with a gap coefficient $\lambda \sim \mathcal{U}(0, 1)$. Finally, these synthetic latent vectors are passed through the frozen decoder to generate synthetic minority images, which are combined with the real data to train a classifier.
