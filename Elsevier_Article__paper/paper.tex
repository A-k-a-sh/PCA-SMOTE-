...
\label{subsec:impl}
We evaluate our approach on two image classification benchmarks: MNIST and CIFAR-10. For MNIST, we use a 100:1 imbalance ratio where Class 9 is reduced to 40 samples (vs. 4000 for the majority class). For CIFAR-10, we establish a 25:1 ratio, reducing the Airplane class to 80 samples (vs. 2000 for the majority class). For downstream classification, we train a lightweight Convolutional Neural Network (SmallCNN) from scratch using the Adam optimizer with Cosine Annealing over 50 epochs. Because global metrics like overall accuracy are insensitive to extreme minority classes, we report the absolute classification accuracy of the target minority class. All experiments are independently repeated across three random seeds (10, 20, and 60) to ensure robustness against initialization artifacts.
