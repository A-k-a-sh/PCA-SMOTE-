...
\label{subsec:impl}
We evaluate our approach on two image classification benchmarks: MNIST and CIFAR-10. While previous works evaluated DeepSMOTE under moderate absolute minority counts (e.g., $n \ge 400$), we establish an extreme imbalance regime to stress-test the latent manifold. For MNIST, we use a 100:1 imbalance ratio, reducing Class 9 to $n=40$ samples. For CIFAR-10, we establish a 25:1 ratio, reducing the Airplane class to $n=80$ samples. 

For downstream classification, we intentionally train a lightweight Convolutional Neural Network (SmallCNN) from scratch. This isolates the direct impact of the synthetic oversampling distribution and avoids confounding variables introduced by heavy architectures (like ResNet-18) or ImageNet pre-training priors. Because global metrics like overall accuracy are insensitive to extreme minority classes, we report the absolute classification accuracy of the target minority class. All experiments are independently repeated across three random seeds (10, 20, and 60) to ensure statistical robustness.
